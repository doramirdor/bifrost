...
// except the last two (create_mcp_oauth_flows_table and
// drop_oauth_config_pkce_columns) and asserts the resulting schema is
// genuinely pre-flows-migration: oauth_user_sessions exists, mcp_oauth_flows
// does not. drop_oauth_config_pkce_columns is excluded too even though it
// isn't the migration under test: it runs after create_mcp_oauth_flows_table
// and touches oauth_configs, not oauth_user_sessions, so leaving it out
// changes nothing this test cares about and keeps the "last registered step"
// assertion below meaningful.
func runPreCreateFlowsMigrationChain(t *testing.T, db *gorm.DB) {
	t.Helper()
	require.GreaterOrEqual(t, len(configstoreMigrationSteps), 2)
	targetStep := configstoreMigrationSteps[len(configstoreMigrationSteps)-2]
	require.Equal(t, []string{"create_mcp_oauth_flows_table"}, targetStep.IDs,
		"expected create_mcp_oauth_flows_table to be the second-to-last registered migration step; "+
			"update the slice bound below if the registry changed")
	preFlowSteps := configstoreMigrationSteps[:len(configstoreMigrationSteps)-2]
