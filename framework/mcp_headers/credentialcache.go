...
// mcp client) binding. NUL-separated so distinct tuples can never collide.
...
func headerCredentialCacheKey(mode schemas.MCPAuthMode, identity, mcpClientID string) string {
	return string(mode) + "\x00" + identity + "\x00" + mcpClientID
}

// splitHeaderCredentialCacheKey is headerCredentialCacheKey's inverse, for
// the scoped eviction predicates. ok is false for a key that is not three
// NUL-joined parts (impossible for keys this package builds).
