...
// in both cases.
function SemanticStatusPanel({ status, isLoading }: { status: SemanticStatusInfo | undefined; isLoading: boolean }) {
	if (isLoading && !status) {
		return (
			...
			<form className="mx-auto w-full max-w-7xl space-y-8" onSubmit={handleSubmit(onValid)} noValidate>
				...
				<div className="space-y-3">
					...
					<Tabs value={mode} onValueChange={(value) => handleModeChange(value as ClassificationMode)}>
						<TabsList className="grid w-full grid-cols-2">
							...
							<TabsTrigger
								...
								disabled={!canUpdate || embeddingProviders.length === 0}
								title={embeddingProviders.length === 0 ? "Configure an embedding-capable provider to enable semantic mode." : undefined}
							>
								Semantic
							</TabsTrigger>
						</TabsList>
					...
					<p className="text-muted-foreground text-xs leading-relaxed">
						{isSemantic ? (
							<>
								Each request is embedded and matched against the tier phrases below as labelled examples. Catches intent that shares no
								words with your lists, at the cost of one embedding call per classified request.
							</>
						) : (
							<>
								Requests are scored against the tier phrases below using keyword matching and message length. No embedding calls, no added
								latency.
							</>
						)}
					</p>
				...
				{isSemantic && (
					<div className="space-y-3">
						...
						<SemanticStatusPanel status={semanticStatus} isLoading={statusLoading} />
						...
						{willReembed && (
							<div className="rounded-sm border border-amber-200 bg-amber-50 p-3 text-xs text-amber-900 dark:border-amber-900 dark:bg-amber-950 dark:text-amber-200">
								<b>Heads up:</b> saving will re-embed all {totalPhrases} tier phrases through the selected provider, which makes{" "}
								{totalPhrases} billable embedding calls. Changing only the similarity floor, timeout, or fallback reuses the existing
								embeddings instead.
							</div>
						)}
											...
											<Controller
												...
												render={({ field }) => (
													<Input
														...
														step={10}
														disabled={!canUpdate}
														value={parseSemanticTimeoutMs(field.value)}
														onChange={(event) => field.onChange(formatSemanticTimeout(Number(event.target.value)))}
														className="font-mono"
													/>
												)}
											/>
				...
				<div className="space-y-3">
					<div className="flex items-baseline gap-2.5">
						...
						{isSemantic && (
							<span className="text-muted-foreground text-xs">
								{liveSemantic?.fallback === "none"
									? "Unused while semantic mode has no fallback — semantic matching picks a tier directly, without a score."
									: "Applies to the lexical fallback only. Semantic matching picks a tier directly, without a score."}
							</span>
						)}
					</div>
				...
				<div className="space-y-3">
					<div className="flex flex-wrap items-baseline justify-between gap-2.5">
						<div className="flex flex-wrap items-baseline gap-2.5">
							...
							<span className="text-muted-foreground text-xs">
								{isSemantic
									? "Embedded as labelled examples — a request is assigned the tier of its nearest phrase. Each phrase must belong to exactly one tier."
