...
func (r *adminDiscoveryRecorder) add(h http.Header) {
	r.mu.Lock()
	defer r.mu.Unlock()
	r.headers = append(r.headers, h.Clone())
}

// headerValues returns every recorded value for name, one per HTTP request
// received. An ephemeral admin-discovery connection issues at least an
// "initialize" and a "tools/list" call over the same transport, both
// carrying whatever headers transport.WithHTTPHeaders configured, so a
// non-empty caller-supplied header should show up on every entry.
...
func TestPerformAdminToolDiscovery_UnsupportedAuthType_ReturnsError(t *testing.T) {
	...
	for _, authType := range authTypes {
		t.Run(string(authType), func(t *testing.T) {
			...
			require.Equal(t, 1, cred.callCount(), "AdminConnectionHeaders should still be resolved before the auth_type switch runs")
		})
	}
}
