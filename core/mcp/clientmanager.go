...
func (m *MCPManager) connectToMCPClient(requestCtx context.Context, config *schemas.MCPClientConfig) error {
	...
	if gateErr != nil {
		...
		m.failConnectAttempt(config, cancel, externalClient, oldCancel, oldConn, needsReauth)
	...
	if listToolsErr != nil {
		m.failConnectAttempt(config, cancel, externalClient, oldCancel, oldConn, false)
	...
	if !exists {
		m.mu.Unlock()
		// Client was removed during connection setup: release both the new
		// handles and any captured old ones to prevent transport/goroutine leaks.
		m.failConnectAttempt(config, cancel, externalClient, oldCancel, oldConn, false)
...
func (m *MCPManager) handleSSEConnectionLost(clientID, clientName string, generation uint64, err error) {
	...
	if !exists || client.ConnGeneration != generation || client.State == schemas.MCPConnectionStateDisabled {
		m.mu.Unlock()
...
// mid-dial leaves the map untouched.
func (m *MCPManager) failConnectAttempt(config *schemas.MCPClientConfig, newCancel context.CancelFunc, newConn *client.Client, oldCancel context.CancelFunc, oldConn *client.Client, needsReauth bool) {
	m.mu.Lock()
	if clientState, exists := m.clientMap[config.ID]; exists && clientState.State != schemas.MCPConnectionStateDisabled {
		clientState.Conn = nil
		clientState.CancelFunc = nil
		clientState.ToolMap = make(map[string]schemas.ChatTool)
		clientState.ToolNameMapping = make(map[string]string)
