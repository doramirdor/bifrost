...
type TokenExchangeIdPResolver interface {
	...
	// Resolve returns the endpoint and grant shape of the active identity
	// provider. Called per exchange (implementations should cache discovery
	// internally) so provider reconfiguration takes effect without
	// restarts.
	Resolve(ctx context.Context) (*TokenExchangeIdP, error)
}
