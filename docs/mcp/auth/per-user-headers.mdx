...
A failed sync keeps the existing tool set and retries on the next cycle. In `config.json`, the field also accepts duration strings such as `"10m"`; a bare number there is interpreted as seconds, unlike the API which takes minutes.
