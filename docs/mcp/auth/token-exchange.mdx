...
Token exchange needs a dedicated application registered at your identity provider — separate from the SSO login application — with the token-exchange (or on-behalf-of) grant enabled and permission to mint tokens for your MCP servers' audiences. Consult your IdP's documentation for how to register this application; the exact grant name and setup steps vary by vendor:
...
5. Fill in:
   ...
   - **Exchange Client ID** — the dedicated IdP application authorized to exchange for this audience
   - **Exchange Client Secret** (optional — omit for public clients)
...
`audience` and `client_id` are required; `client_secret` and `scopes` are optional. `client_id` and `client_secret` support `env.VAR_NAME` and `vault.path` references — plain values also work (encrypted at rest, redacted in API responses).
...
| `client_id` | string | Yes | The dedicated exchange application's client ID (env./vault. references supported) |
| `client_secret` | string | No | The exchange application's secret; omit for public clients (env./vault. references supported) |
| `scopes` | string[] | No | Scopes requested on the exchanged token; include `offline_access` for a self-renewing admin discovery credential |
