...
def openapi_operations(spec: dict[str, Any]) -> list[tuple[str, str, bool, str]]:
    operations: list[tuple[str, str, bool, str]] = []
    for path, path_item in spec.get("paths", {}).items():
        ...
        for method in HTTP_METHODS:
            ...
            is_legacy_governance_alias = str(
                operation.get("x-bifrost-successor", "")
            ).startswith("/api/governance/")
            operations.append(
                (
                    str(tags[0]),
                    f"{method.upper()} {path}",
                    is_legacy_governance_alias,
                    summary,
                )
            )
    return operations
...
def build_navigation(
    ...
    governance["pages"] = [
        ...
        if not (
            isinstance(page, dict) and page.get("group") == "Deprecated APIs"
        )
    ]
    ...
    governed_openapi_pages = {
        page
        for _, page, is_legacy, _ in operations
        if page.split(" ", 1)[1].startswith("/api/governance/") and not is_legacy
    }
    ...
    for tag, page, is_legacy, summary in operations:
        if is_legacy:
            continue
    ...
    for tag, page, is_legacy, _ in operations:
        if page in governance_set or is_legacy:
            continue
        pages_by_tag.setdefault(tag, []).append(page)

    generated_groups = [
        {"group": tag, "pages": pages} for tag, pages in pages_by_tag.items()
    ]

    api_tab["openapi"] = OPENAPI_SOURCE
    api_tab["groups"] = [authentication, governance, *generated_groups]

    final_pages = list(iter_page_entries(api_tab.get("groups", [])))
    final_set = set(final_pages)
    visible_operation_set = {
        page for _, page, is_legacy, _ in operations if not is_legacy
    }
